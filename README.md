...
- `image` is a path like `assets/img/sponsors/pods.png`. If the file is missing,
  the sponsor's `name` shows as text instead — that's the current state of the
  **Moon Ridge** entry until a logo file is added at `assets/img/sponsors/moon-ridge.png`.
- `url` is optional; set it to make the logo a link.
- Logos render ~30px tall — use a transparent or white-background PNG, roughly 3:1 to 5:1 wide.
...
- The home page automatically shows the **soonest upcoming game** (or the next
  game whose date hasn't passed). No upcoming games → it shows an off-season message.
